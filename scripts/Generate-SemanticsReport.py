...
from tests.test_sprint531_exposure import build_exposure_fixture


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", required=True)
    parser.add_argument("--software-exposure", action="store_true")
    ...
    try:
        generator = (
            build_exposure_fixture(case)
            if args.software_exposure
            else build_semantics_fixture(case)
        )
