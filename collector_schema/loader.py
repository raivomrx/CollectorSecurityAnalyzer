...
def _parse_v2_document(data: dict[str, Any]) -> CollectorDocument:
    ...
    return CollectorDocument(
        ...
        device=DeviceIdentity(
            ...
            computer_name=device.get("computerName") or device.get("hostname"),
            fqdn=device.get("fqdn"),
            domain_or_workgroup=device.get("domainOrWorkgroup"),
            entra_joined=device.get("entraJoined"),
            entra_tenant_id=device.get("entraTenantId") or device.get("tenantId"),
            entra_device_id=device.get("deviceId"),
            current_user_sid=device.get("currentUserSid"),
            execution_integrity_level=device.get("executionIntegrityLevel"),
        ),
        ...
        software=SoftwareInventoryEvidence(
            items=data.get("software", {}).get("items", []) if isinstance(data.get("software"), dict) else [],
            collection_status=str(data.get("software", {}).get("collectionStatus", "NOT_EVALUATED")) if isinstance(data.get("software"), dict) else "NOT_EVALUATED",
            records_collected=int(data.get("software", {}).get("recordsCollected", 0)) if isinstance(data.get("software"), dict) else 0,
            errors=data.get("software", {}).get("errors", []) if isinstance(data.get("software"), dict) else [],
        ),
